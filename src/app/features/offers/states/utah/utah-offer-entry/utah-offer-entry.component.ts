...
@Component({
  ...
  templateUrl:
    './utah-offer-entry.component.html',

  styleUrl:
    './utah-offer-entry.component.scss',

  changeDetection:
    ChangeDetectionStrategy.OnPush,
})
export class UtahOfferEntryComponent
  implements OnInit {
  ...
  protected readonly currentVersion =
    signal<OfferVersion<UtahOfferTerms> | null>(null);
  ...
  async ngOnInit(): Promise<void> {
    try {
      ...
      if (stateCode !== 'UT') {
        throw new Error(
          'The Utah offer process can only be used for property located in Utah.'
        );
      }
      ...
      if (
        ...
        throw new Error(
          'This listing requires a Utah contract form that is not yet enabled. NavStreet currently supports the OREC Residential Sale form for qualifying residential property.'
        );
      }
    } catch (error) {
      this.setError(
        error,
        'The Utah offer process could not be opened.'
      );
    } finally {
  ...
  protected async onSubmitRequested(
    ...
    try {
      ...
      this.setError(
        error,
        'Your Utah agreement could not be prepared.'
      );
    } finally {
  ...
  private async loadOfferSession(
    ...
    if (!offer || !version) {
      throw new Error(
        'The Utah offer draft could not be loaded.'
      );
    }

    if (
      offer.stateCode !== 'UT' ||
      version.stateCode !== 'UT' ||
      version.terms.stateCode !== 'UT'
    ) {
      throw new Error(
        'The saved offer does not contain a Utah contract.'
      );
    }
    ...
    if (
      ...
      throw new Error(
        'The existing draft uses a different Utah contract form.'
      );
    }
    ...
    const save =
      this.offerService
        ...
        .catch(error => {
          this.setError(
            error,
            'Your latest Utah offer changes could not be saved.'
          );
