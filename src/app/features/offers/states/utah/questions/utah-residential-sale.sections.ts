...
/** Terms are original NavStreet terms; these questions do not fill Utah's state PDF. */
export const UTAH_RESIDENTIAL_SALE_SECTIONS: readonly OfferSectionDefinition[] = [
  { id: 'parties', title: 'Buyers and sellers', description: 'Review the parties copied from the listing and account.', questions: [] },
  { id: 'property', title: 'Property and water', questions: [
    ...
    yn('propertyItems.waterRightsIncluded', 'Are any water rights or shares included?', 'Select whether any water rights or shares are included.'),
    { id: 'excluded-water', type: 'textarea', label: 'Water rights excluded or transfer details', fieldPath: 'propertyItems.excludedWaterRights', helpText: 'Describe specific exclusions or special transfer requirements.' },
  ] },
  ...
  { id: 'deadlines', title: 'Contract deadlines', description: 'Unless the parties agree otherwise, times are 5:00 p.m. Mountain Time on each date.', questions: [
    { id: 'seller-date', type: 'date', label: 'Seller disclosure deadline', fieldPath: 'deadlines.sellerDisclosureDate', validation: required('Set the seller disclosure deadline.') },
  ...
  { id: 'disclosures', title: 'Seller disclosures', questions: [
    { id: 'seller-meth-statement', type: 'yes_no', label: 'Seller reports known current methamphetamine contamination', fieldPath: 'disclosures.sellerReportsCurrentMethContamination', readOnly: true, validation: required('The seller must answer the contamination question on the listing.') },
    { id: 'condition-status', type: 'single_choice', label: 'Seller property condition statement', fieldPath: 'disclosures.propertyConditionStatus', validation: required('Select the condition statement status.'), options: [
      { value: 'received', label: 'Received' }, { value: 'pending', label: 'Seller to deliver by disclosure deadline' },
    ] },
    ...
    { id: 'hoa-status', type: 'single_choice', label: 'Association documents', fieldPath: 'disclosures.hoaDocumentsStatus', validation: required('Select the association document status.'), options: [
