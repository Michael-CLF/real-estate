...
@Component({
  ...
  templateUrl:
    './oklahoma-offer-entry.component.html',

  styleUrl:
    './oklahoma-offer-entry.component.scss',
...
export class OklahomaOfferEntryComponent
  ...
  protected readonly currentVersion =
    signal<OfferVersion<OklahomaOfferTerms> | null>(null);
  ...
  async ngOnInit(): Promise<void> {
    try {
      ...
      if (stateCode !== 'OK') {
        throw new Error(
          'The Oklahoma offer process can only be used for property located in Oklahoma.'
        );
      }
      ...
      if (
        ...
        throw new Error(
          'This listing requires an Oklahoma contract form that is not yet enabled. NavStreet currently supports the OREC Residential Sale form for qualifying residential property.'
        );
      }
    } catch (error) {
      this.setError(
        error,
        'The Oklahoma offer process could not be opened.'
      );
    } finally {
  ...
  protected onDraftChanged(
    change: OklahomaOfferDraftChange
  ): void {
    if (
      !this.currentOffer() ||
      !this.currentVersion()
