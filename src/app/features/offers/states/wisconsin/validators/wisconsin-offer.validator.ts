...
export class WisconsinOfferValidator implements StateOfferValidator<WisconsinOfferTerms> {
  readonly stateCode = 'WI' as const;
  validate(terms: WisconsinOfferTerms, buyers: OfferParty[], sellers: OfferParty[], context: OfferValidationContext): OfferValidationResult {
    ...
    if (terms.stateCode !== 'WI' || terms.contractType !== 'navstreet_wisconsin_residential_sale_2026') error('form', 'Select the NavStreet Wisconsin residential agreement.');
    if (terms.property.state !== 'WI') error('property.state', 'The listing must be in Wisconsin.');
    ...
    if (terms.propertyItems.fixturesIncluded === null) error('propertyItems.fixturesIncluded', 'Select whether installed fixtures are included.');
