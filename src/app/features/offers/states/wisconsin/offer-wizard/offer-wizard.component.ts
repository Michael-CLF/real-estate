...
import {
  updateWisconsinOfferTerms,
} from '../services/wisconsin-offer-terms-updater';
...
export class OfferWizardComponent {
  ...
  readonly initialTerms =
    input<WisconsinOfferTerms | null>(null);

  readonly saving = input(false);
  readonly submitting = input(false);

  readonly draftChanged =
    output<WisconsinOfferDraftChange>();
  ...
  protected readonly terms =
    signal<WisconsinOfferTerms | null>(null);
  ...
  protected readonly contractDefinition = computed(
    () => {
      ...
      return terms
        ? WISCONSIN_OFFER_PACKAGE.contracts[
            terms.contractType
          ]
        : null;
    }
  ...
  >(
    () => {
      ...
      const result = WISCONSIN_OFFER_PACKAGE.validate(
        terms,
        this.draftBuyers(),
        this.sellers(),
        {
      ...
      for (const [status, path, documentType, label] of [
        ...
        [terms.disclosures.propertyConditionStatus, 'disclosures.propertyConditionStatus', 'wisconsin-real-estate-condition-report', 'seller-signed Wisconsin real estate condition report'],
        [terms.disclosures.hoaDocumentsStatus, 'disclosures.hoaDocumentsStatus', 'wisconsin-association-documents', 'association documents'],
      ] as const) {
