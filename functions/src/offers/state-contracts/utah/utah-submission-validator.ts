...
export function validateUtahSubmission(input: ValidateStateSubmissionInput<UtahOfferTermsDocument>): void {
  ...
  requireValue(t.propertyItems.waterRightsIncluded !== null, 'Specify whether water rights are included.');
  ...
  requireValue(t.disclosures.propertyConditionStatus !== 'unselected' && t.disclosures.hoaDocumentsStatus !== 'unselected', 'Select disclosure and association status.');
  ...
  requireValue(t.delivery.timeZone === 'America/Denver' && t.delivery.electronicDeliveryAuthorized === true, 'Electronic delivery consent and Mountain time are required.');
